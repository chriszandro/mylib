...
class system(object):
    ...
    Class:  
    '''
    def __init__(self, computation_data, system_data="None", occupation_data="None", unoccupation_data="None",
                 franck_data="None", prob_data="None"):
        """
        :type occupation_data: csv data from nano switch. object
        :type system_data: csv data from nano switch. It contains the wave functions and the used dvr grid
        :type computation_data: computation Data from the nano switch program containing observable statistics
        """
        import numpy as np
        '''
            System
    ...
    def plot_system(self, axes, switch=False, density=False, start=0, end=20, amplification=10):
        ...
        for i in range(start, end):
            ...
            if density:
                axes.plot(self.grid, self.energy_occupied[i] + (
                    (self.wavefunction[:, i] - self.energy_occupied[i]) ** 2) * amplification, linewidth=1,
                          linestyle=lw,
                          label='State ' + str(i))
            else:
                axes.plot(self.grid,
                          self.energy_occupied[i] + (self.wavefunction[:, i] - self.energy_occupied[i]) * amplification,
                          linewidth=1, linestyle=lw, label='State ' + str(i))
    ...
    def plot_termscheme(self, axes, arrowlist, y0=0, delta_y=0.1, distance=1, cap=10, inputcolor='b'):
        ...
        for element in arrowlist:
            ...
            axes.arrow(y0, self.energy_unoccupied[element[0]], 0, self.energy_unoccupied[element[0]] + increment,
                       head_width=0.05, fc="k", ec="k", width=0.02, head_length=0.02, length_includes_head=True,
                       color=inputcolor)
            y0 = y0 + 0.2
    ...
    def plot_position_t(self, axes, scale='lin', style='-', colorp="b", siunits=True, name=''):
        ...
        if siunits:
            ...
            axes.plot(self.parameter * self.au2second, self.position * self.bohrtoangstrom, linestyle=style,
                      color=colorp, lw='2', label=name)
    ...
    def plot_population(self, axes, pop_number=10, scale='lin', name=''):
        ...
        for i in range(1, pop_number):
            axes.plot(self.occupation[0] * self.au2second, self.occupation[i], linestyle='-', lw='2',
                      label='State' + str(i - 1))
        return axes
...
class density(object):
    ...
    def plotdensity_sgn(self, ax, index=0, colorb="b", labelb="no label", linestyleb="-"):
        import numpy as np
        ax.plot(self.grid, np.sign(self.density[index]), color=colorb, label=labelb, linestyle=linestyleb,
                linewidth=1.5)
        return ax
...
class frankmatrix(object):
    ...
    def put_pair(self, axes, state=0, *args):
        ...
        axes.plot(self.gate_voltages, self.frank_matrix[state],
                  label=str(state) + " > " + str(state + self.quanta) + " ( " + str(self.frank_shift) + " )", *args)
    ...
    def put_transition(self, ax, quanta, end_state=None):
        ...
        for i in range(0, end_state):
            ax.plot(self.gate_voltages, self.thresholds[quanta]["threshold"][i],
                    label=str(self.thresholds[quanta]["pair"][i][0]) + " > " + str(
                        self.thresholds[quanta]["pair"][i][1]))
